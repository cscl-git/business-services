	...
	public void setAssignments(ResultSet rs, Employee currentEmployee) {
		try {
			...
			if(!StringUtils.isEmpty(rs.getString("assignment_uuid")) && !ids.contains(rs.getString("assignment_uuid"))) {
			    ...
			    .govtOrderNumber(rs.getString("assignment_govtordernumber")).reportingTo(rs.getString("assignment_reportingto")).isHOD(rs.getBoolean("assignment_ishod")).isPrimaryAssignment(rs.getBoolean("assignment_isprimaryassignment"))
				.isCurrentAssignment(rs.getBoolean("assignment_iscurrentassignment")).postDetailId(rs.getLong("assignment_postdetailid")).tenantid(rs.getString("assignment_tenantid")).auditDetails(auditDetails).build();
